...
from saleor.account.models import Invitation
...
def resolve_invitations(info):
    requesting_user = get_user_or_app_from_context(info.context)
    qs = Invitation.objects.using(get_database_connection_name(info.context))
    if (
        requesting_user
        and not requesting_user.is_staff
...
def resolve_order(info, id):
    ...
    if order is None:
        raise ValidationError(
            "Can not find the requested order.", code="ORDER_NOT_FOUND"
        )
    check_is_owner_or_has_one_of_perms(user, order.user, OrderPermissions.MANAGE_ORDERS)
    return order
